...
from envs.cube.env import CustomMuJoCoEnv
from envs.cube import mjcf_utils, controllers
from envs.cube.lie import SE3, SO3
...
class ManipSpaceEnv(CustomMuJoCoEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ob_type: Observation type. Either 'states', 'pixels', or 'both'.
        ...
        self._effector_down_rotation = SO3.SO3(np.asarray([0.0, 1.0, 0.0, 0.0]))
        ...
        assert ob_type in ['states', 'pixels', 'both']
    ...
    def observation_space(self):
        ...
        if self._ob_type == 'pixels':
            ex_ob = self.get_pixel_observation()
            return Box(low=0, high=255, shape=ex_ob.shape, dtype=ex_ob.dtype)
        elif self._ob_type == 'states':
            ex_ob = self.compute_latent_observation()
            return Box(low=-np.inf, high=np.inf, shape=ex_ob.shape, dtype=ex_ob.dtype)
        else:
            ex_img = self.get_pixel_observation()
            ex_lat = self.compute_latent_observation()

            return Dict({
                'pixels': Box(low=0, high=255, shape=ex_img.shape, dtype=ex_img.dtype),
                'latent': Box(low=-np.inf, high=np.inf, shape=ex_lat.shape, dtype=ex_lat.dtype),
            })
    ...
    def build_mjcf_model(self):
        ...
        if self._ob_type == 'pixels' or self._ob_type == 'both':
    ...
    def post_compilation(self):
        ...
        pinch_pose = SE3.SE3.from_rotation_and_translation(
            rotation=SO3.SO3.from_matrix(self._data.site_xmat[self._pinch_site_id].reshape(3, 3)),
            translation=self._data.site_xpos[self._pinch_site_id],
        )
        attach_pose = SE3.SE3.from_rotation_and_translation(
            rotation=SO3.SO3.from_matrix(self._data.site_xmat[self._attach_site_id].reshape(3, 3)),
    ...
    def reset(self, goal_idx=None, options=None, *args, **kwargs):
        if self._mode == 'task':
            # Set the task goal.
            if options is None:
                options = {}
    ...
    def initialize_arm(self):
        ...
        rotz = SO3.SO3.from_z_radians(yaw)
        eff_ori = rotz @ cur_ori

        # Solve for initial joint positions using IK.
        T_wp = SE3.SE3.from_rotation_and_translation(eff_ori, eff_pos)
    ...
    def set_control(self, action):
        ...
        effector_yaw = SO3.SO3.from_matrix(
        ...
        target_effector_orientation = (
            SO3.SO3.from_z_radians(a_ori)
            @ SO3.SO3.from_z_radians(effector_yaw)
        ...
        target_effector_orientation = SO3.SO3.from_z_radians(yaw) @ self._effector_down_rotation
        ...
        self._target_effector_pose = SE3.SE3.from_rotation_and_translation(
    ...
    def compute_ob_info(self):
        ...
        ob_info['proprio/effector_yaw'] = np.array(
            [SO3.SO3.from_matrix(self._data.site_xmat[self._pinch_site_id].copy().reshape(3, 3)).compute_yaw_radians()]
        )

...
from envs.cube.lie.SO3 import SO3
from envs.cube.manipspace_env import ManipSpaceEnv
...
class CubeEnv(ManipSpaceEnv):
    ...
    def __init__(self, env_type, permute_blocks=False, randomize_initial=True, *args, **kwargs):
        ...
        Args:
            ...
            randomize_initial: Whether to randomize the initial positions of the blocks.
            *args: Additional arguments to pass to the parent class.
            **kwargs: Additional keyword arguments to pass to the parent class.
        """
        self._env_type = env_type
        self._permute_blocks = permute_blocks
        self._randomize_initial = randomize_initial
    ...
    def initialize_episode(self):
        ...
        if self._mode == 'data_collection':
            ...
            for i in range(self._num_cubes):
                ...
                obj_ori = SO3.from_z_radians(yaw).wxyz.tolist()
            ...
            if not self._randomize_initial:
            ...
            for i in range(self._num_cubes):
                ...
                self._data.joint(f'object_joint_{i}').qpos[3:] = SO3.identity().wxyz.tolist()
                self._data.mocap_pos[self._cube_target_mocap_ids[i]] = goal_xyzs[i]
                self._data.mocap_quat[self._cube_target_mocap_ids[i]] = SO3.identity().wxyz.tolist()
    ...
    def switch_target_block(self, return_info=True):
        ...
        if len(top_blocks) == 0:
            # fallback: choose any block
            self._target_block = 0
        else:
            # Prefer a different block than the current target if possible
            choices = list(set(top_blocks) - {getattr(self, '_target_block', None)})
            if len(choices) == 0:
                choices = top_blocks
            self._target_block = int(self.np_random.choice(choices))
    ...
    def set_new_target(self, return_info=True, p_stack=0.5):
        ...
        # Compute the top blocks + pick one of the top cubes as the target.
        top_blocks = self.switch_target_block(return_info=False)
        ...
        yaw = self.np_random.uniform(0, 2 * np.pi)
        tar_ori = SO3.from_z_radians(yaw).wxyz.tolist()
    ...
    #             [SO3(wxyz=self._data.joint(f'object_joint_{i}').qpos[3:]).compute_yaw_radians()]
    #         )

    def add_object_info(self, ob_info):
        # Cube positions and orientations (always)
        for i in range(self._num_cubes):
            ob_info[f'privileged/block_{i}_pos'] = self._data.joint(f'object_joint_{i}').qpos[:3].copy()
            ob_info[f'privileged/block_{i}_quat'] = self._data.joint(f'object_joint_{i}').qpos[3:].copy()
            ob_info[f'privileged/block_{i}_yaw'] = np.array(
        ...
        if self._mode == 'data_collection':
            ...
            ob_info['privileged/target_block_yaw'] = np.array(
                [SO3(wxyz=self._data.mocap_quat[target_mocap_id]).compute_yaw_radians()]
            )
        else:
            # TASK MODE: keep current target_block if set, otherwise default to 0
    ...
    def compute_latent_observation(self):
    ...
    def compute_observation(self):
        ...
        elif self._ob_type == 'states':
            return self.compute_latent_observation()
        else:
            obs_img = self.get_pixel_observation()
            obs_latent = self.compute_latent_observation()
            return {'pixels': obs_img, 'latent': obs_latent}
...
class CubeWrapper(gym.Wrapper):
    ...
    def _img_to_chw(self, img):
        arr = np.asarray(img)
        return einops.rearrange(arr.astype(np.float32) / 255.0, "H W C -> C H W")
    ...
    def reset(self, goal_idx=None, *args, **kwargs):
        ...
        self.moved = 0
        self.entered = 0
        return np.expand_dims(img, axis=0) # add v dimension

    def step(self, action):
        obs, reward, terminated, truncated, info = self.env.step(action)
        # make obs pixels
        if isinstance(obs, dict):
            raw_pixels = obs.get("pixels", obs)
        else:
            raw_pixels = obs
        info["image"] = self.env.render()
        info["all_completions_ids"] = []
        obs = self._img_to_chw(raw_pixels)
...
for env_id in [
    ...
    if env_id in registration.registry:
        del registration.registry[env_id]
...
registration.register(
    id="cube-single-v1",
    entry_point="envs.cube.cube_env:CubeEnv",

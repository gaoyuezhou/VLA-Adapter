    ...
    try:
        while True:
            ...
            if cmd == "step":
                ...
                p.send(env.is_success(data[0], data[1]))  # TODO: check
            elif cmd == "update_env":
...
class SubprocEnvWorker(EnvWorker):
    ...
    def is_success(self, goal_state, cur_state):
        self.parent_remote.send(["is_success", (goal_state, cur_state)])
        return self.parent_remote.recv()
...
class BaseVectorEnv(object):
    ...
    def is_success(self, goal_state, cur_state):
        ...
        for i in range(self.env_num):
            s = self.workers[i].is_success(goal_state[i], cur_state[i])
            successes.append(s)
        return successes

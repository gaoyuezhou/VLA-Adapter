            ...
            for b_i, b in enumerate(blocks):
                if self._in_target[t_i][b_i] == -1:
                    ...
                    if dist < self.goal_dist_tolerance:
                        ...
                        # self.entered += 1
                        # Only credit `entered` once per block, so it caps at 2
                        # (there are 2 blocks). A single block that reaches both
                        # targets must not be counted twice.
                        block_already_placed = any(
                            self._in_target[ti][b_i] == 0
                            for ti in range(len(targets))
                            if ti != t_i
                        )
                        if not block_already_placed:
                            self.entered += 1
                        reward += 0.49

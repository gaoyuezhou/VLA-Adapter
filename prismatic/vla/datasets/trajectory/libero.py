"""LIBERO goal trajectory dataset. Adapted from patch_policy."""
import torch
import einops
import numpy as np
...
class LiberoGoalDataset(TrajectoryDataset):
    ...
    #          demo_{i}
    #              agentview_image.pth
    #              robot0_eye_in_hand_image.pth
    #              robot0_joint_pos.npy
    #              robot0_eef.npy
    #              robot0_gripper_pos.npy
    #              object_states.npy
    #              actions.npy
    def __init__(
            self,
        ...
        # pad state dimension to same length
        MAX_DIM = 128
    ...
    def get_task_name_for_demo(self, idx):
        """Extract human-readable task name from the demo's directory structure."""
        demo_path = self.demos[idx]
        task_dir_name = demo_path.parent.name
        # Directory names are like "put_the_bowl_on_the_plate" -> "put the bowl on the plate"
        return task_dir_name.replace("_", " ")

    def get_frames(self, idx, frames):
        demo = self.demos[idx]
        agentview_obs = torch.load(str(demo / "agentview_image.pth"))
        robotview_obs = torch.load(str(demo / "robot0_eye_in_hand_image.pth"))
